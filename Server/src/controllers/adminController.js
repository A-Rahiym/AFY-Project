...
import {
  assignRoomToStudentByChoices,
  batchAssignRoomsToStudents,
...
export const assignRoomToStudentController = async (req, res) => {
  ...
  try {
    ...
    if (assignmentResult) {
      ...
      return res.status(200).json({
        ...
        message:
          ...
          `Room assigned successfully to student ${studentId}.`,
    ...
    if (
      error.message.includes("Student with ID") &&
    ...
    return res
      ...
      .json({
        success: false,
        error: "Failed to assign room due to a server error.",
      });
  }
};
...
export const assignRoomToStudentsBatchController = async (req, res) => {
  ...
  if (!studentIds || !Array.isArray(studentIds) || studentIds.length === 0) {
    return res
      ...
      .json({
        success: false,
        error: "An array of student IDs is required in the request body.",
      });
  }
  ...
  try {
    ...
    const successfulAssignments = batchResults.filter(
      (r) => r.status === "success"
    );
    ...
    return res.status(200).json({
      ...
      summary: {
        ...
        failedStudents: failedAssignments.map((r) => ({
          id: r.studentId,
          message: r.message,
        })),
        errorStudents: errorAssignments.map((r) => ({
          id: r.studentId,
          message: r.message,
        })),
      },
    ...
    console.error(
      "Error in assignRoomToStudentsBatchController (outer catch):",
      error.message
    );
    return res
      .status(500)
      .json({
        success: false,
        error: "Failed to complete batch assignment due to a server error.",
      });
  }
};
...
export const fetchPaymentStatus = async (req, res) => {
  try {
    const data = await getPaymentStatusSummary();
    res.status(200).json(data);
  } catch (error) {
    console.error('RPC Error - payment status:', error);
    res.status(500).json({ error: 'Failed to fetch payment status summary' });
  }
};

export const fetchHostelOccupancy = async (req, res) => {
  try {
    const data = await getHostelOccupancySummary();
    res.status(200).json(data);
  } catch (error) {
    console.error('RPC Error - hostel occupancy:', error);
    res.status(500).json({ error: 'Failed to fetch hostel occupancy summary' });
  }
};